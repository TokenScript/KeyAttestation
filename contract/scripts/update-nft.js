...
(async ()=>{
    ...
    try {
        const newproxyLPNFT = await upgrades.upgradeProxy(currentNFTProxyAddress, LPNFT.connect(mainDeployKey));
        await newproxyLPNFT.deployed();

        console.log("[PROXY & LOGIC CONTRACTS] --> LaPrairieNFTTestNet Logic Contract Upgraded");
    } catch (e) {
...
// npx hardhat run scripts/update-nft.js --network