...
(async ()=>{
    ...
    const newRoyaltyValue = 10 * 100;  // 10% ie 10 * 100
    ...
    try {
		await proxyLPNFT.connect(mainDeployKey).setRoyaltyValue(newRoyaltyValue);

        console.log('[PROXY & LOGIC CONTRACTS] --> LaPrairieNFTTestNet Royalty percentage updated to ', newRoyaltyValue/100, "%");
    } catch (e) {
...
// npx hardhat run scripts/update-nft-royalty-value.js --network rinkeby