...
(async ()=>{
    ...
    if (ownerAddress.toLowerCase() !== mainDeployKey.address.toLowerCase()) {
        console.log(`must reserve using contract owner key: (Owner: ${ownerAddress} vs your key: ${mainDeployKey.address}), execution stopping...`);
        return;
    }

    try {
		await proxyLPNFT.connect(mainDeployKey).reserve(reserveCount, carlaReceiveAddress);

        console.log(`LaPrairieNFTTestNet reserved ${reserveCount} to address ${carlaReceiveAddress}`, e);
    } catch (e) {
        ethersDebugMessages(`LaPrairieNFTTestNet reserve ${reserveCount} FAILED`, e);
    }
...
// npx hardhat run scripts/reserve-mint.js --network rinkeby